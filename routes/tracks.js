...
router.get('/withInfos', function(res, res, next) {
  ...
  models.Track.findAll().then(function(trackModelList) {
    ...
    trackModelList.forEach(function(track) {
      var promise = models.Session.count({where: {track_id: track.id, date_session: models.sequelize.fn('date_format', new Date(new Date() - 24 * 60 * 60 * 1000), "YYYY-MM-DD") }}).then(function(result) {
